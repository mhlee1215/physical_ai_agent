...
class Checkpoint25Test(TestCase):
    ...
    def test_reference_table_contains_leaderboard_and_partial_run_row(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("OpenCabinet", text)
            self.assertIn("OpenDrawer", text)
            self.assertIn("0/60, 0.0%", text)
            self.assertIn("Current Scale-Up Blocker", text)
            self.assertIn("TurnOnMicrowave", text)
            self.assertIn("valid_categories", text)
