...
class MyCobotNexusEnvTest(unittest.TestCase):
    def test_contract_declares_reset_step_render_surface(self) -> None:
        ...
        self.assertIn("official_pro_adaptive_gripper", contract["task_objects"])
...
""".strip(),
    ...
    def test_scene_builder_can_use_official_ros1_pro_adaptive_gripper_meshes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            model_path = tmp_path / "xml" / "mycobot.xml"
            scene_path = tmp_path / "render" / "scene.xml"
...
""".strip(),
        ...
        self.assertIn("official_pro_adaptive_gripper", names)
