...
| ManiSkill3 selected Franka tasks | A recent STARE paper reports a direct SmolVLA row on four ManiSkill3 Franka tasks, so it is table-backed outside LIBERO/Meta-World. | Table 2 reports `SmolVLA (fine-tuning)` with Avg `51.5%`: StackCube `12.7`, PushCube `86.3`, PullCube `90.7`, LiftPegUpright `16.3`. It states Octo and SmolVLA use `1000` trajectory samples for SFT per task. | Not immediately comparable to the released SmolVLA checkpoints. It requires task-specific SFT data/training and a working ManiSkill3 renderer/runtime. Current direct ManiSkill path still needs RunPod renderer validation. | Medium. Good for low-level manipulation and stage-aware recovery once the policy/training lane exists. | Add as a future table-backed training benchmark, not as the next released-checkpoint evaluation. |
...
| Future training benchmark | ManiSkill3 selected Franka tasks | 4 tasks, task-specific SFT before eval | task success | pending | SmolVLA fine-tuning Avg `51.5`; StackCube `12.7`, PushCube `86.3`, PullCube `90.7`, LiftPegUpright `16.3` | Table-backed, but not comparable to released SmolVLA until task-specific SFT and ManiSkill3 runtime are implemented |
...
5. treat ManiSkill3 selected Franka tasks as a separate table-backed
   ...
   SFT rather than a released general checkpoint.
