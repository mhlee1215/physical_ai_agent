...
| Future training benchmark | ManiSkill3 selected Franka tasks | 4 tasks, task-specific SFT before eval | task success | runtime smoke passed; policy eval pending | SmolVLA fine-tuning Avg `51.5`; StackCube `12.7`, PushCube `86.3`, PullCube `90.7`, LiftPegUpright `16.3` | Table-backed, but not comparable to released SmolVLA until task-specific SFT is implemented |
...
5. treat ManiSkill3 selected Franka tasks as a separate table-backed
   ...
   passes for all four selected tasks.
