...
| ManiSkill3 selected Franka tasks | A recent STARE paper reports a direct SmolVLA row on four ManiSkill3 Franka tasks, so it is table-backed outside LIBERO/Meta-World. | Table 2 reports `SmolVLA (fine-tuning)` with Avg `51.5%`: StackCube `12.7`, PushCube `86.3`, PullCube `90.7`, LiftPegUpright `16.3`. It states Octo and SmolVLA use `1000` trajectory samples for SFT per task. | RunPod runtime audit passed with `mani_skill==3.0.1` and `sapien==3.0.3`: `PushCube-v1`, `PullCube-v1`, `StackCube-v1`, and `LiftPegUpright-v1` all reset, stepped, and rendered RGB frames. The SFT feasibility audit downloaded official demos for all four STARE tasks. `PushCube-v1` and `StackCube-v1` have `1000` successful motion-planning trajectories; `PullCube-v1` has `1024` successful RL trajectories; `LiftPegUpright-v1` has `1015` successful RL `pd_ee_delta_pose` trajectories and `993` successful RL `pd_joint_delta_pos` trajectories. The audit also converted `PushCube-v1` action-only trajectories to LeRobot format, replayed one episode with RGB observations, and converted that RGB replay to a LeRobot dataset with `observation.images.base_camera`, `observation.state`, and `action`. A local motion-planning generation attempt segfaulted and wrote a truncated `.h5`, so use official demos or replay rather than relying on local generator output on this Pod. | Medium. Good for low-level manipulation and stage-aware recovery once the policy/training lane exists. | Promote to the next table-backed training benchmark candidate after Meta-World parity. Runtime and official-demo availability blockers are mostly cleared; remaining blocker is full task-specific SmolVLA SFT/eval across all four STARE tasks, with careful disclosure that demo source/control mode is not identical for every task. |
...
| Future training benchmark | ManiSkill3 selected Franka tasks | 4 tasks, task-specific SFT before eval | task success | runtime smoke passed; official demos found for all four tasks with at least `993` successful trajectories per selected source; `PushCube-v1` action-only and 1-episode RGB replay conversion to LeRobot passed | SmolVLA fine-tuning Avg `51.5`; StackCube `12.7`, PushCube `86.3`, PullCube `90.7`, LiftPegUpright `16.3` | Table-backed, but not comparable to released SmolVLA until task-specific SFT/eval is implemented for the four selected tasks |
...
5. treat ManiSkill3 selected Franka tasks as a separate table-backed
   ...
   source, and the `PushCube-v1` path proves the first data-preparation step:
   official trajectories are convertible to LeRobot format; RGB replay plus
   LeRobot conversion works on a 1-episode smoke.
