...
| Active training benchmark | ManiSkill3 `PushCube-v1` | count1000 official-demo replay, 9000 SFT steps, 50 eval episodes | task success | `58.0%` (`29/50`), shared runner, qpos state, horizon `100`, postprocessor includes `UnnormalizerProcessorStep` | STARE SmolVLA fine-tuning PushCube `86.3%` | Paper-like data scale for one task, but still `-28.3pp`; use as SFT calibration baseline, not agentic-wrapper evidence |
