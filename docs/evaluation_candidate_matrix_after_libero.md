...
| Parity ablation | Meta-World `very_hard` | 5 tasks, 10 trials/task | task success | seed `0` corrected-rename run `20.0`; seed `1000` run `26.0`; seed `1000` + `empty_cameras=0` run `26.0`; standalone `batch_size=10` run `36.0` | SmolVLA Table 2 Very Hard `60.0` | Standalone probes can improve, but the full-run result does not preserve the gain |
