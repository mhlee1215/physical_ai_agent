...
| Future training benchmark | ManiSkill3 selected Franka tasks | 4 tasks, task-specific SFT before eval | task success | runtime smoke passed; official demos found for all four tasks with at least `993` successful trajectories per selected source; 1-episode RGB replay and LeRobot conversion passed for all four tasks; pretrained `lerobot/smolvla_base` 1-step train smoke passed for all four tasks with `rename_map`; shared-runner `PushCube-v1` count10/100-step eval smoke `0/3` with `UnnormalizerProcessorStep` applied | SmolVLA fine-tuning Avg `51.5`; StackCube `12.7`, PushCube `86.3`, PullCube `90.7`, LiftPegUpright `16.3` | Table-backed, but not comparable to STARE until SFT/eval is scaled to paper-like trajectory counts |
