...
| ManiSkill3 selected Franka tasks | A recent STARE paper reports a direct SmolVLA row on four ManiSkill3 Franka tasks, so it is table-backed outside LIBERO/Meta-World. | Table 2 reports `SmolVLA (fine-tuning)` with Avg `51.5%`: StackCube `12.7`, PushCube `86.3`, PullCube `90.7`, LiftPegUpright `16.3`. It states Octo and SmolVLA use `1000` trajectory samples for SFT per task. | RunPod runtime audit passed with `mani_skill==3.0.1` and `sapien==3.0.3`: `PushCube-v1`, `PullCube-v1`, `StackCube-v1`, and `LiftPegUpright-v1` all reset, stepped, and rendered RGB frames. The SFT feasibility audit downloaded the official `PushCube-v1` motion-planning demo, confirmed it contains `1000` successful trajectories, converted the action-only trajectory to LeRobot format, replayed one episode with RGB observations, and converted that RGB replay to a LeRobot dataset with `observation.images.base_camera`, `observation.state`, and `action`. A local motion-planning generation attempt segfaulted and wrote a truncated `.h5`, so use official demos or replay rather than relying on local generator output on this Pod. | Medium. Good for low-level manipulation and stage-aware recovery once the policy/training lane exists. | Promote to the next table-backed training benchmark candidate after Meta-World parity. Runtime and data-conversion blockers are mostly cleared for `PushCube`; remaining blocker is full task-specific SmolVLA SFT/eval across all four STARE tasks. |
...
| Future training benchmark | ManiSkill3 selected Franka tasks | 4 tasks, task-specific SFT before eval | task success | runtime smoke passed; `PushCube-v1` official 1000-demo download and LeRobot conversion passed; 1 RGB replay converted with `observation.images.base_camera` | SmolVLA fine-tuning Avg `51.5`; StackCube `12.7`, PushCube `86.3`, PullCube `90.7`, LiftPegUpright `16.3` | Table-backed, but not comparable to released SmolVLA until task-specific SFT/eval is implemented for the four selected tasks |
...
5. treat ManiSkill3 selected Franka tasks as a separate table-backed
   ...
   passes for all four selected tasks, and the `PushCube-v1` official-demo
   path now proves the first data-preparation step: `1000` official
   trajectories are downloadable and convertible to LeRobot format; RGB replay
   plus LeRobot conversion works on a 1-episode smoke.
