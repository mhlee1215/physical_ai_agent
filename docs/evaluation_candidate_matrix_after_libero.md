...
| Parity ablation | Meta-World `very_hard` | 5 tasks, 10 trials/task | task success | seed `0` corrected-rename run `20.0`; seed `1000` run `26.0`; seed `1000` + `empty_cameras=0` run `26.0` | SmolVLA Table 2 Very Hard `60.0` | Seed helps only modestly; empty-camera padding is not the leading cause |
