...
- Loop tests must record analyzer artifacts by default. Keep
  ...
  preserves raw Qwen payloads, policy input images, robot frames,
  per-iteration videos, rollout config, and action-chunk metadata. Use
  `--no-record-loop-artifacts` only for explicitly labeled lightweight smoke or
  debugging runs, not for validation evidence.
