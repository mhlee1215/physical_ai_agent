...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run Gate 8 for the myCobot 320 adaptive gripper: table pregrasp, "
            "slow close, and short lift with two-pad contact retained."
        )
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("_workspace/mycobot_adaptive_grasp_lift_smoke"),
    )
    parser.add_argument(
        "--asset-root",
        type=Path,
