...
from physical_ai_agent.sim.mycobot_nexus_env import (
    ...
    ADAPTIVE_FINGER_PAD_FRICTION,
    ADAPTIVE_FINGER_PAD_SIZE,
    ADAPTIVE_LEFT_FINGER_PAD_PARENT,
    ADAPTIVE_LEFT_FINGER_PAD_POS,
    ADAPTIVE_RIGHT_FINGER_PAD_PARENT,
    ADAPTIVE_RIGHT_FINGER_PAD_POS,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Verify adaptive gripper collision proxy pads are derived from "
            "validated official finger link frames."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("_workspace/mycobot_320_adaptive_collision_proxy_verify"),
    )
    return parser
...
def verify_adaptive_collision_proxy(
    *,
    ...
    expected = _expected_proxy_specs(official_gripper_root)
...
def _expected_proxy_specs(official_gripper_root: Path) -> dict[str, dict[str, Any]]:
    specs = {}
    mesh_root = official_gripper_root / OFFICIAL_320_ADAPTIVE_GRIPPER_MESH_RELATIVE_PATH
...
def _render_markdown(report: CollisionProxyReport) -> str:
    lines = [
        "# myCobot 320 Adaptive Collision Proxy Gate",
        "",
...
def _render_svg(report: CollisionProxyReport) -> str:
    ...
    parts = [
        ...
        _svg_text(36, 52, "myCobot 320 adaptive collision proxy gate", 30, "700"),
