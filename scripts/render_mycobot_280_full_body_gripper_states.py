...
def _draw_header(frame: np.ndarray, state: str, view: str, description: str, command: float, cube_pos: np.ndarray | None) -> None:
    ...
    cv2.putText(frame, f"gripper command={command:+.2f}; corrected pad proxies; {cube_text}", (18, 92), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (45, 45, 45), 1, cv2.LINE_AA)


def _draw_legend(frame: np.ndarray, *, show_cube: bool) -> None:
    entries = [("arm/body", None), ("gripper base", "gripper_base_visual_0"), ("distal fingers", "gripper_left1_visual_0"), ("support links", "gripper_left2_visual_0"), ("left pad", "left_finger_pad"), ("right pad", "right_finger_pad")]
    if show_cube:
