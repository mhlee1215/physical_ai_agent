...
def _write_reference_table(path: Path) -> None:
    lines = [
        ...
        "`CloseFridge` row is a LeRobot-documented single-task quick-iteration run",
        "with 20 episodes per task. It is useful evidence that the SmolVLA/RoboCasa",
        "pipeline runs, but it is not the same scale as the RoboCasa365 50-task",
        "leaderboard. Only the full `atomic_seen`, `composite_seen`, and",
        "`composite_unseen` runs should be compared directly against leaderboard",
        "split or overall values.",
        "",
        "Sources:",
        "",
