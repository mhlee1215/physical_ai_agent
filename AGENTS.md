...
- Whenever the Robot Experiment Manager / dataset viewer is started, opened, or reported, prepare and provide all three verified links together: localhost, same-Wi-Fi mobile, and external access. Keep the viewer and external tunnel under `launchctl`; verify `/api/datasets` through the external URL before reporting it.
...
- Every new SO101 dataset must be declared in `configs/so101/dataset_generation/*.json`. Dataset generation is not complete until every selected split passes the mandatory distribution stage and stores `meta/distribution/distribution.json`, `.md`, and `.html`. The pipeline order is machine statistics JSON -> canonical Markdown -> visual HTML, and completion must reject a Markdown/HTML pair whose recorded Markdown SHA-256 does not match. Cube datasets that randomize orientation must gate both absolute yaw and robot-relative yaw `(object_yaw - angle_from_base) mod symmetry_period`; absolute yaw span alone is not evidence that different cube faces are shown to the robot. Unless an explicit physical-feasibility exception is declared, placement angle must not deterministically select the robot-relative cube face. Then the final `completion:registry-viewer` stage runs `scripts/verify_so101_dataset_completion.py`, restarts the launchctl-managed Robot Experiment Manager to load the current schema, finds every selected split in `/api/datasets`, and loads episode 0/frame 0 with camera1, camera2, and prompt through `/api/frame`.
- Primitive phase datasets are supervision slices and must not declare or materialize phase-specific closed-loop tests. Keep exactly one official end-to-end loop test on the parent/full-task dataset, sourced from its validation split and judged by final task success, unless the user explicitly requests a temporary diagnostic exception.
- New SO101 recipes use Pydantic `schema_version: 2` and must declare a discriminated `source.mode`: `from_scratch` creates simulator state and teacher trajectories without external inputs; `from_spawn_catalog` consumes only a seed-free placement catalog (camera-bin `[x,y]` or typed workspace XY + candidate yaw + sampling metadata) and generates new state/trajectory/images/seeds; `from_existing_dataset` declares exact source roots for full dataset-derived regeneration, render derivatives, or an explicit `episode_subset` materialization. Placement catalogs must never contain source frames/actions/states/seeds. An episode subset must preserve retained frame/action/state values, record source episode provenance, and write a new append-only root. When only placement is reused, extract a catalog and do not list the old dataset as the new recipe source. New `grip_the_cube_v1` recipes declare typed `common.inspection_gates`: exactly one jaw-line versus contacted-face-normal geometry gate is required and at most one camera2 visual-alignment gate may be added. Camera2 gating must use constructive wrist-roll refinement plus a pre-close/early-close probe before full episode export; post-export filtering alone is not a valid generation contract. Existing schema-v1 recipes remain readable for reproduction and must not be rewritten in place.
